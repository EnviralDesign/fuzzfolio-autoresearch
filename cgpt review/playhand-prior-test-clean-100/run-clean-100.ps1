...
$OutDir = Join-Path $Root "runs\derived\playhand-prior-test-clean-100"
...
"[$startedAt] Starting clean 100-seed Play Hand confirmation batch" | Add-Content -Path $BatchLog -Encoding UTF8
...
foreach ($seed in $Seeds) {
    ...
    if (Test-Path $seedLog) {
        $text = Get-Content -Path $seedLog -Raw
    }
...
"[$finishedAt] Finished clean 100-seed Play Hand confirmation batch completed=$completed failed=$failed skipped=$skipped" | Add-Content -Path $BatchLog -Encoding UTF8
