...
$OutDir = Join-Path $Root "runs\derived\playhand-prior-test-clean-50"
...
"[$startedAt] Starting clean 50-seed Play Hand batch" | Add-Content -Path $BatchLog -Encoding UTF8
"Command template: $commandTemplate" | Add-Content -Path $BatchLog -Encoding UTF8

foreach ($seed in $Seeds) {
    $seedLabel = "{0:D2}" -f $seed
...
"[$finishedAt] Finished clean 50-seed Play Hand batch completed=$completed failed=$failed skipped=$skipped" | Add-Content -Path $BatchLog -Encoding UTF8
