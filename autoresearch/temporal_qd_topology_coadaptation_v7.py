"""Experiment-only topology co-adaptation matrix v7.

v4 proved complete P/T/E/TE blocks. v7 reconstructs both pair legs, refuses to
...
COADAPTATION_SCHEMA = "temporal_qd_topology_coadaptation_matrix_v7"
COADAPTATION_MODE = "frozen_complete_2x2_insert_setup_then_topology_local_event_pair_receipts_v7"
...
RECEIPT_KEYS = (
    ...
    "pairCompileStatus",
    "canonicalCompiledIdentities",
    "historicalParentPairIdentitySha256",
    "compiledV3MatchesHistoricalParent",
    "frozenPairPayloadSha256",
...
COMPILED_IDENTITY_KEYS = (
    ...
    "longNativeValidationReportSha256",
    "shortNativeValidationReportSha256",
)
PAIR_COMPILE_STATUS_COMPLETE = "canonical_frozen_pair_compiled_and_natively_validated"
PAIR_COMPILE_STATUS_INCOMPLETE = "incomplete_block_not_materialized"
PAIR_COMPILE_STATUS_UNAVAILABLE = "module_sides_reconstructed_pair_compiler_unavailable"
SETTLING_KEYS = (
    "kind",
...
SUCCESS_VALUES = {
    "schemaVersion": "temporal_qd_topology_coadaptation_success_v7",
...
def _validate_parents(value: Any) -> list[dict[str, str]]:
    if not isinstance(value, list) or not value:
        raise TemporalDiscoveryContractError("topology coadaptation v7 requires frozen parents")
    parents: list[dict[str, str]] = []
    seen: set[str] = set()
    has_archive = False
    for item in value:
        row = _exact_object(item, PARENT_KEYS, "topology coadaptation v7 parent")
        candidate_id = row["candidateId"]
        role = row["role"]
        if not isinstance(candidate_id, str) or not candidate_id.strip():
            raise _unexpected("topology coadaptation v7 parent")
        if role not in PARENT_ROLES:
            raise TemporalDiscoveryContractError("topology coadaptation v7 parent role is invalid")
        if candidate_id in seen:
            raise TemporalDiscoveryContractError(f"topology coadaptation v7 repeats parent {candidate_id}")
    ...
    if not has_archive:
        raise TemporalDiscoveryContractError("topology coadaptation v7 requires at least one archive parent")
    return parents


def _validate_panels(value: Any) -> dict[str, Any]:
    row = _exact_object(value, PANEL_KEYS, "topology coadaptation v7 panelIdentities")
    ...
    if tuple(row["replicationPanelIds"] or ()) != ("panel-1", "panel-2"):
        raise TemporalDiscoveryContractError("topology coadaptation v7 replication panels drifted")
    _require_text(
        row["replicationRole"],
...
def _validate_slots(
    ...
    *,
    ...
    if not isinstance(value, list) or not value:
        raise TemporalDiscoveryContractError("topology coadaptation v7 requires slots")
    ...
    seen: set[str] = set()
    for item in value:
        row = _exact_object(item, SLOT_KEYS, "topology coadaptation v7 slot")
        slot_id = row["slotId"]
        if not isinstance(slot_id, str) or not slot_id or slot_id in seen:
            raise _unexpected("topology coadaptation v7 slot")
        seen.add(slot_id)
        arm = row["arm"]
        if arm not in ARMS:
            raise TemporalDiscoveryContractError("topology coadaptation v7 slot arm drifted")
        parent_id = row["parentCandidateId"]
        if parent_id not in parent_by_id:
            raise TemporalDiscoveryContractError("topology coadaptation v7 slot parent drifted")
        side = row["side"]
        if side not in SIDES:
            raise TemporalDiscoveryContractError("topology coadaptation v7 slot side drifted")
        if not isinstance(row["blockId"], str) or not row["blockId"]:
            raise _unexpected("topology coadaptation v7 blockId")
        if arm == ARM_P:
...
def _validate_blocks(
    ...
    *,
    ...
    if not isinstance(value, list) or not value:
        raise TemporalDiscoveryContractError("topology coadaptation v7 requires blocks")
    ...
    seen: set[str] = set()
    for item in value:
        row = _exact_object(item, BLOCK_KEYS, "topology coadaptation v7 block")
        block_id = row["blockId"]
        if not isinstance(block_id, str) or not block_id or block_id in seen:
            raise _unexpected("topology coadaptation v7 block")
        seen.add(block_id)
        parent_id = row["parentCandidateId"]
        side = row["side"]
        if parent_id not in parent_by_id or side not in SIDES:
    ...
    *,
    ...
    for item in value:
        ...
        if block_id in complete_ids:
            ...
            if copied["nativeValidationRan"] is not True:
                raise TemporalDiscoveryContractError("complete-block receipts must run native validation")
            ...
            identities = copied.get("canonicalCompiledIdentities")
            if not isinstance(identities, Mapping):
                raise TemporalDiscoveryContractError("complete-block receipts must bind canonical compiled identities")
            identity_row = _exact_object(identities, COMPILED_IDENTITY_KEYS, "canonicalCompiledIdentities")
            for key in COMPILED_IDENTITY_KEYS:
                _require_sha(identity_row[key], f"canonicalCompiledIdentities.{key}")
            if copied["frozenPairIdentitySha256"] != identity_row["frozenPairIdentitySha256"]:
                raise TemporalDiscoveryContractError("frozenPairIdentitySha256 must equal canonicalCompiledIdentities.frozenPairIdentitySha256")
            if copied["frozenPairPayloadSha256"] != identity_row["frozenPairPayloadSha256"]:
                raise TemporalDiscoveryContractError("frozenPairPayloadSha256 must equal canonicalCompiledIdentities.frozenPairPayloadSha256")
            if copied["pairProfileSha256"] != identity_row["pairProfileSha256"]:
                raise TemporalDiscoveryContractError("pairProfileSha256 drifted from canonical compiled identities")
            if copied["normalizedProfileSnapshotSha256"] != identity_row["normalizedProfileSnapshotSha256"]:
                raise TemporalDiscoveryContractError("normalizedProfileSnapshotSha256 drifted from canonical compiled identities")
            if copied["canonicalPairCompilerAuthoritySha256"] != identity_row["pairCompilerAuthoritySha256"]:
                raise TemporalDiscoveryContractError("pair compiler authority drifted from canonical compiled identities")
            if copied["canonicalPairCompileReportSha256"] != identity_row["validationReportSha256"]:
                raise TemporalDiscoveryContractError("pair compile report drifted from canonical compiled identities")
            if copied["nativeValidationAuthoritySha256"] != identity_row["nativeAuthoritySha256"]:
                raise TemporalDiscoveryContractError("native authority drifted from canonical compiled identities")
            if copied["nativeValidationReportSha256"] != identity_row["validationReportSha256"]:
                raise TemporalDiscoveryContractError("native validation report drifted from canonical compiled identities")
            if arm == ARM_P:
                if copied["historicalParentPairIdentitySha256"] is not None:
                    _require_sha(copied["historicalParentPairIdentitySha256"], "historicalParentPairIdentitySha256")
                if copied["compiledV3MatchesHistoricalParent"] is not True:
                    raise TemporalDiscoveryContractError("P receipt must prove compiled v3 identities match the historical parent")
            else:
                if copied["historicalParentPairIdentitySha256"] is not None:
                    raise TemporalDiscoveryContractError("non-parent receipts must not carry historical parent pair identity")
                if copied["compiledV3MatchesHistoricalParent"] is not None:
                    raise TemporalDiscoveryContractError("non-parent receipts must not claim historical v3 parent match")
            ...
            if copied["canonicalCompiledIdentities"] is not None or copied["frozenPairIdentitySha256"] is not None:
                raise TemporalDiscoveryContractError("incomplete-block receipts cannot claim canonical FrozenPair identities")
            if copied["frozenPairPayloadSha256"] is not None:
                raise TemporalDiscoveryContractError("incomplete-block receipts cannot claim FrozenPair payloads")
...
def validate_topology_coadaptation_matrix_v7(contract: Mapping[str, Any]) -> dict[str, Any]:
    row = _exact_object(contract, ROOT_KEYS, "topology coadaptation v7")
    ...
    if row["mutationDepth"] != 1:
        raise TemporalDiscoveryContractError("topology coadaptation v7 mutationDepth drifted")
    ...
    if tuple(row["arms"] or ()) != ARMS:
        raise TemporalDiscoveryContractError("topology coadaptation v7 arms drifted")
    ...
    if success != SUCCESS_VALUES:
        raise TemporalDiscoveryContractError("topology coadaptation v7 successCalculation drifted")
    ...
    if instrumentation != EVALUATION_INSTRUMENTATION_VALUES:
        raise TemporalDiscoveryContractError("topology coadaptation v7 evaluationInstrumentation drifted")
    ...
    if design != expected_design:
        raise TemporalDiscoveryContractError("topology coadaptation v7 designScope drifted")
    body = {
    ...
    if row["contractSha256"] != expected:
        raise TemporalDiscoveryContractError("topology coadaptation v7 identity drift")
    body["contractSha256"] = expected
    return body


def topology_coadaptation_v7_from_config(config: Mapping[str, Any] | None) -> dict[str, Any] | None:
    if not isinstance(config, Mapping):
        return None
    overlay = config.get("topologyCoadaptationMatrix")
    if overlay is None:
        return None
    return validate_topology_coadaptation_matrix_v7(overlay)


def attach_topology_coadaptation_matrix_v7(
    ...
    config["topologyCoadaptationMatrix"] = validate_topology_coadaptation_matrix_v7(matrix)
    config["configSha256"] = canonical_sha256(config)
    return config


def build_topology_coadaptation_matrix_v7(
    *,
    ...
    return validate_topology_coadaptation_matrix_v7(body)


def _receipt_shell_v7(receipt_v6: Mapping[str, Any]) -> dict[str, Any]:
    row = {key: receipt_v6.get(key) for key in RECEIPT_KEYS}
    row.setdefault("canonicalCompiledIdentities", None)
    row.setdefault("historicalParentPairIdentitySha256", None)
    row.setdefault("compiledV3MatchesHistoricalParent", None)
    row.setdefault("frozenPairPayloadSha256", None)
    return row


def topology_spec_v7_from_v6(
    spec_v6: Mapping[str, Any],
    *,
    ...
    shell_receipts = [_receipt_shell_v7(item) for item in (receipts if receipts is not None else spec_v6["materializationReceipts"])]
    return build_topology_coadaptation_matrix_v7(
        parents=spec_v6["parents"],
        rotating_evidence_sha256=spec_v6["panelIdentities"]["rotatingEvidenceSha256"],
        topology_plans=spec_v6["topologyPlans"],
        event_primitives=spec_v6["eventPrimitives"],
        slots=slots if slots is not None else spec_v6["slots"],
        blocks=blocks if blocks is not None else spec_v6["blocks"],
        materialization_receipts=shell_receipts,
    )
...
def apply_topology_parity_mutation_v7(contract: Mapping[str, Any], mutation_id: str) -> dict[str, Any]:
    ...
    elif mutation_id == "success_policy_drift":
        mutated["successCalculation"]["usefulProgressiveInnovationRequiresTeGreaterThanP"] = False
    elif mutation_id == "native_ran_false_while_compiled":
        index = _first_eligible_index(mutated, arm=ARM_TE)
        receipts[index]["nativeValidationRan"] = False
    else:
...
PARITY_MUTATION_IDS = (
    ...
    "success_policy_drift",
    "native_ran_false_while_compiled",
)


def evaluate_topology_parity_case_v7(contract: Mapping[str, Any], mutation_id: str | None) -> dict[str, Any]:
    payload = contract if mutation_id is None else apply_topology_parity_mutation_v7(contract, mutation_id)
    try:
...
def build_topology_parity_corpus_v7(contract: Mapping[str, Any]) -> dict[str, Any]:
    cases = [evaluate_topology_parity_case_v7(contract, None)]
    for mutation_id in PARITY_MUTATION_IDS:
        cases.append(evaluate_topology_parity_case_v7(contract, mutation_id))
    body = {
        "schemaVersion": "temporal_qd_topology_coadaptation_python_rust_parity_corpus_v7",
        "canonicalFixtureAccepted": cases[0]["accepted"] is True,
        "adversarialCount": len(PARITY_MUTATION_IDS),
        "cases": cases,
        "pythonValidator": "validate_topology_coadaptation_matrix_v7",
        "rustValidator": "topology_coadaptation_matrix_v7::validate",
        "doNotLaunch": True,
    }
