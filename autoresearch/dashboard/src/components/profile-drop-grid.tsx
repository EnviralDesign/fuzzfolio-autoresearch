  ...
  return (
    <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
      {items.map((item, index) => (
        <Card key={item.attempt_id} className="border-border/60 bg-card/80 shadow-2xl shadow-black/20">
          ...
          <CardContent className="space-y-4">
            {item.png_url ? (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="block overflow-hidden rounded-3xl border border-border/60 bg-background/70"
              >
                <img
                  src={item.png_url}
