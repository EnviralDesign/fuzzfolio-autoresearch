"""V38 follow-up audit v6: slot-bound receipts, fail-closed pair compile, useful-innovation risk rule.
...
from .temporal_qd_topology_coadaptation_v6 import (
    ARM_E,
    ARM_P,
    ARM_T,
    ARM_TE,
    ...
    build_topology_coadaptation_matrix_v6,
    build_topology_parity_corpus_v6,
...
from .temporal_qd_v38_followup_authorities_v6 import (
    attempt_canonical_pair_compile_and_native_validation,
    bind_frozen_v38_policies,
    build_resource_confirmation_authority_v6,
    build_resource_near_two_plan_design_v6,
    build_resource_one_plan_design_v6,
    build_resource_selected_pair_receipts_v6,
    build_standalone_receipt_set_v6,
    build_topology_confirmation_authority_v6,
    build_topology_inspected_task_authority_v6,
)
...
EVENT_INSERT_SCHEMA_V6 = "temporal_qd_v38_directional_event_insert_forensic_v6"
MULTI_PANEL_SCHEMA_V6 = "temporal_qd_v38_multipanel_suboperation_v6"
ARCHIVE_FORENSIC_SCHEMA_V6 = "temporal_qd_v38_cumulative_event_child_archive_forensic_v6"
COVERED_MONTHS = 36.0
...
def build_archive_forensic_v6(archive_v4: Mapping[str, Any]) -> dict[str, Any]:
    ...
    direction = _direction_selection_from_row(member, policy=DEFAULT_DIRECTION_SELECTION_POLICY)
    binding = []
    ...
    focus_v5 = {
        **focus,
        ...
        "exactSupportMetrics": {
            ...
            "robustThresholdSource": "live_module_default_not_independently_rebound_from_frozen_v38_archive_bytes",
        },
...
def build_event_and_multipanel_v6(
    *,
...
def build_topology_spec_v6(spec_v4: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return build_topology_coadaptation_matrix_v6(
...
def decision_memo_v6(
    *,
    ...
    return "\n".join(
        [
            "# V38 follow-up decision memo v6",
            "",
            ...
            "",
            "## Pair compile and native validation",
            "",
            "Dashboard-owned FrozenPair.compile and native validation remain unavailable: no frozen pair-run config is bound in the follow-up artifact set. Complete-block receipts reconstruct both program legs, preserve the opposite side, bind receiptId to slotId, and refuse pair/native labels without actual reports. Synthetic reconstructed pair identities are not launch authority.",
            "",
            f"Complete 2x2 blocks remain **{complete}** case studies. `insert_setup` is a timing/staging mutation; occupancy, freshness, trade-count, cost, and support/direction/quality instrumentation fields are frozen unevaluated.",
            "",
            "## Useful innovation",
            "",
            ...
            "",
            (
                f"Topology inspected authority is 12 complete-block pair receipts x 12 windows = 144 tasks. "
                f"Future untouched topology confirmation is 48 tasks with window identities still unbound. "
...
def run_audit_v6(*, output_dir: Path, v38_root: Path | None = None, catalog_path: Path | None = None) -> dict[str, Path]:
    ...
    event, multipanel = build_event_and_multipanel_v6(event_v4=event_v4, multipanel_v4=multipanel_v4)
    archive = build_archive_forensic_v6(archive_v4)
    ...
    topology = build_topology_spec_v6(spec_v4)
    validate_topology_coadaptation_matrix_v6(topology)
    ...
    assert topology_coadaptation_v6_from_config({"schemaVersion": "temporal_qd_pair_generation_v2"}) is None
    pair_attempt = attempt_canonical_pair_compile_and_native_validation()
    policies = bind_frozen_v38_policies(topology=topology)
    archive["frozenV38PolicyBinding"] = policies
    archive = _self_hash({key: value for key, value in archive.items() if key != "reportSha256"})
    inspected_authority = build_topology_inspected_task_authority_v6(topology=topology, inventory_v2=inventory)
    topology_confirmation = build_topology_confirmation_authority_v6(topology=topology)
    resource_confirmation = build_resource_confirmation_authority_v6(inventory_v2=inventory)
    one_plan = build_resource_one_plan_design_v6(proposal_v3=proposal)
    near_two = build_resource_near_two_plan_design_v6(inventory_v2=inventory)
    selected = build_resource_selected_pair_receipts_v6(inventory_v2=inventory, one_plan_cells=proposal["cells"])
    parity = build_topology_parity_corpus_v6(topology)
    parity_markdown = "\n".join(
        [
    ...
    memo = decision_memo_v6(archive=archive, inventory=inventory, proposal=proposal, topology=topology)
    outputs: dict[str, tuple[Mapping[str, Any] | None, str]] = {
        "v38-directional-event-insert-forensic-v6.json": (
            event,
            "# V38 directional event-insert forensic v5\n\nResealed v4 mechanism partition. No new market evaluation.\n",
        ),
        "v38-multipanel-suboperation-v6.json": (
            multipanel,
        ...
        "v38-cumulative-event-child-archive-forensic-v6.json": (
            archive,
        ...
        "topology-case-study-inspected-task-authority-v6.json": (
            inspected_authority,
            "# Topology case-study inspected-task authority v6\n\n12 complete-block pair receipts x 12 windows = 144 inspected tasks. Do not launch.\n",
        ),
        "topology-future-untouched-confirmation-authority-v6.json": (
            topology_confirmation,
            "# Topology future untouched confirmation authority v6\n\n48 confirmation tasks for complete-block pair receipts. Do not launch.\n",
        ),
        "resource-future-untouched-confirmation-authority-v6.json": (
            resource_confirmation,
            "# Resource future untouched confirmation authority v6\n\nSeparate from the topology case study. Do not launch.\n",
        ),
        "resource-suboperation-selected-pair-receipts-v6.json": (
            selected,
            "# Resource selected pair receipts v6\n\n58 one-plan cells plus 5 clones. Canonical pair compile unavailable. Do not launch.\n",
        ),
        "resource-suboperation-one-plan-design-v6.json": (
            one_plan,
            "# Resource one-plan design v6\n\nCoverage, not repeatability. Do not launch.\n",
        ),
        "resource-suboperation-near-two-plan-design-v6.json": (
            near_two,
            "# Resource near-two-plan design v6\n\nAvailability-backed second plans. Do not launch.\n",
        ),
        "canonical-pair-compile-attempt-v6.json": (
            pair_attempt,
            "# Canonical pair-compile attempt v6\n\nFail closed: frozen pair-run config is not bound.\n",
        ),
        "topology-coadaptation-python-rust-parity-corpus-v6.json": (
            parity,
            parity_markdown,
        ),
    }
    ...
    for name, (payload, markdown) in outputs.items():
        ...
        written[name] = path
    memo_path = output_dir / "decision-memo-v6.md"
    memo_path.write_text(memo, encoding="utf-8", newline="\n")
    written["decision-memo-v6.md"] = memo_path
    receipts_path = output_dir / "topology-coadaptation-materialization-receipts-v6.json"
    receipts_payload = build_standalone_receipt_set_v6(topology=topology, pair_attempt=pair_attempt)
    receipts_path.write_text(
        canonical_json(receipts_payload) + "\n",
        encoding="utf-8",
        newline="\n",
    )
    written["topology-coadaptation-materialization-receipts-v6.json"] = receipts_path
    readme_v6 = output_dir / "README-v6.md"
    readme_v6.write_text(
        "\n".join(
            [
                "# V38 evolve-everything follow-up artifacts v6",
                "",
                "Generated locally from frozen v4/v5 artifacts and current source. No new market evaluation. v1-v5 files in this folder are unchanged except the shared README appendix.",
                "",
                "| File | Task |",
                "| --- | --- |",
                "| `v38-directional-event-insert-forensic-v6.json` / `.md` | A |",
                "| `v38-multipanel-suboperation-v6.json` / `.md` | A |",
                "| `v38-cumulative-event-child-archive-forensic-v6.json` / `.md` | E |",
                "| `topology-coadaptation-matrix-spec-v6.json` / `.md` | A-D |",
                "| `topology-coadaptation-materialization-receipts-v6.json` | B/I |",
                "| `topology-case-study-inspected-task-authority-v6.json` / `.md` | F |",
                "| `topology-future-untouched-confirmation-authority-v6.json` / `.md` | F |",
                "| `resource-future-untouched-confirmation-authority-v6.json` / `.md` | F/H |",
                "| `resource-suboperation-selected-pair-receipts-v6.json` / `.md` | H |",
                "| `resource-suboperation-one-plan-design-v6.json` / `.md` | H |",
                "| `resource-suboperation-near-two-plan-design-v6.json` / `.md` | H |",
                "| `canonical-pair-compile-attempt-v6.json` / `.md` | A |",
                "| `topology-coadaptation-python-rust-parity-corpus-v6.json` / `.md` | C |",
                "| `decision-memo-v6.md` | memo |",
                "| `README-v6.md` | index |",
                "",
                "Regenerate v6 with:",
                "",
    ...
    written["README-v6.md"] = readme_v6
...
def main(argv: list[str] | None = None) -> int:
    ...
    written = run_audit_v6(output_dir=args.output_dir, v38_root=args.v38_root, catalog_path=args.catalog)
