...
use temporal_qd_kernel::topology_coadaptation_matrix_v7::{
    COADAPTATION_SCHEMA, from_generation_config, validate,
};

const FIXTURE: &str = include_str!(
    "../../../../../research/temporal-qd/v38-followup/topology-coadaptation-matrix-spec-v7.json"
);
const CORPUS: &str = include_str!(
    "../../../../../research/temporal-qd/v38-followup/topology-coadaptation-python-rust-parity-corpus-v7.json"
);

fn load_fixture() -> Value {
    let trimmed = FIXTURE.trim();
    assert!(!trimmed.is_empty(), "v7 spec fixture must be emitted before rust tests");
...
fn apply_mutation(spec: &Value, mutation_id: &str) -> Value {
    ...
    match mutation_id {
        ...
        "success_policy_drift" => {
            mutated["successCalculation"]["usefulProgressiveInnovationRequiresTeGreaterThanP"] = json!(false);
        }
        "native_ran_false_while_compiled" => {
            let index = first_eligible_index(&mutated, "topology_then_topology_local_event");
            mutated["materializationReceipts"][index]["nativeValidationRan"] = json!(false);
        }
