//! Experiment-only topology co-adaptation overlay v7.
//!
...
pub const COADAPTATION_SCHEMA: &str = "temporal_qd_topology_coadaptation_matrix_v7";
pub const COADAPTATION_MODE: &str = "frozen_complete_2x2_insert_setup_then_topology_local_event_pair_receipts_v7";
...
const COMPILED_IDENTITY_KEYS: [&str; 13] = [
    "frozenPairIdentitySha256",
    "frozenPairPayloadSha256",
    "pairProfileSha256",
    "normalizedProfileSnapshotSha256",
...
pub fn validate(payload: &Value) -> Result<Value> {
    let fields = object(payload, "topology coadaptation v7")?;
    exact_keys(fields, &ROOT_KEYS, "topology coadaptation v7")?;
    ...
    if fields.get("mutationDepth") != Some(&json!(1)) {
        return Err(contract("topology coadaptation v7 mutationDepth drifted"));
    }
    ...
    let parent_rows = parents_value
        .as_array()
        .ok_or_else(|| contract("topology coadaptation v7 requires frozen parents"))?;
    if parent_rows.is_empty() {
        return Err(contract("topology coadaptation v7 requires frozen parents"));
    }
    let mut parents = BTreeMap::new();
    let mut has_archive = false;
    for item in parent_rows {
        let parent = object(item, "topology coadaptation v7 parent")?;
        exact_keys(parent, &PARENT_KEYS, "topology coadaptation v7 parent")?;
        let candidate_id = text(parent, "candidateId", "parent")?.to_owned();
        if candidate_id.is_empty() || parents.contains_key(&candidate_id) {
            return Err(contract("topology coadaptation v7 parent drifted"));
        }
    ...
    if !has_archive {
        return Err(contract("topology coadaptation v7 requires at least one archive parent"));
    }
    ...
    {
        let row = object(item, "topology coadaptation v7 slot")?;
        exact_keys(row, &SLOT_KEYS, "topology coadaptation v7 slot")?;
        let slot_id = text(row, "slotId", "slot")?.to_owned();
        if slot_id.is_empty() || slots.contains_key(&slot_id) {
            return Err(unexpected("topology coadaptation v7 slot"));
        }
        let arm = text(row, "arm", "slot arm")?;
        if !ARMS.contains(&arm) {
            return Err(contract("topology coadaptation v7 slot arm drifted"));
        }
        let parent_id = text(row, "parentCandidateId", "slot parent")?;
        let side = text(row, "side", "slot side")?;
        if !parents.contains_key(parent_id) || (side != "long" && side != "short") {
            return Err(contract("topology coadaptation v7 slot parent/side drifted"));
        }
        let block_id = text(row, "blockId", "blockId")?.to_owned();
        if block_id.is_empty() {
            return Err(unexpected("topology coadaptation v7 blockId"));
        }
    ...
    let blocks = fields
        .get("blocks")
        .and_then(Value::as_array)
        .ok_or_else(|| contract("topology coadaptation v7 requires blocks"))?;
    if blocks.is_empty() {
        return Err(contract("topology coadaptation v7 requires blocks"));
    }
    let mut seen_blocks = BTreeSet::new();
    for item in blocks {
        let row = object(item, "topology coadaptation v7 block")?;
        exact_keys(row, &BLOCK_KEYS, "topology coadaptation v7 block")?;
        let block_id = text(row, "blockId", "block")?.to_owned();
        if !seen_blocks.insert(block_id.clone()) {
            return Err(unexpected("topology coadaptation v7 block"));
        }
    ...
    for item in receipts_value {
        ...
        if complete_blocks.contains(&block_id) {
            ...
            require_bool(row, "nativeValidationRan", true, "complete-block receipts must run native validation")?;
            require_text(
                row,
                "pairCompileStatus",
                "canonical_frozen_pair_compiled_and_natively_validated",
                "complete-block pairCompileStatus drifted",
            )?;
            let identities_value = row
                .get("canonicalCompiledIdentities")
                .ok_or_else(|| contract("complete-block receipts must bind canonical compiled identities"))?;
            let identities = object(identities_value, "canonicalCompiledIdentities")?;
            exact_keys(identities, &COMPILED_IDENTITY_KEYS, "canonicalCompiledIdentities")?;
            for key in COMPILED_IDENTITY_KEYS {
                require_sha(identities.get(key).ok_or_else(|| unexpected(key))?, &format!("canonicalCompiledIdentities.{key}"))?;
            }
            for key in [
                "frozenPairIdentitySha256",
                "frozenPairPayloadSha256",
            ...
            if text(row, "frozenPairIdentitySha256", "frozenPairIdentitySha256")?
                != text(identities, "frozenPairIdentitySha256", "identities.frozenPairIdentitySha256")?
            {
                return Err(contract(
                    "frozenPairIdentitySha256 must equal canonicalCompiledIdentities.frozenPairIdentitySha256",
                ));
            }
            if text(row, "frozenPairPayloadSha256", "frozenPairPayloadSha256")?
                != text(identities, "frozenPairPayloadSha256", "identities.frozenPairPayloadSha256")?
            {
                return Err(contract(
                    "frozenPairPayloadSha256 must equal canonicalCompiledIdentities.frozenPairPayloadSha256",
                ));
            }
            if text(row, "pairProfileSha256", "pairProfileSha256")?
                != text(identities, "pairProfileSha256", "identities.pairProfileSha256")?
            {
                return Err(contract("pairProfileSha256 drifted from canonical compiled identities"));
            }
            if text(row, "normalizedProfileSnapshotSha256", "normalizedProfileSnapshotSha256")?
                != text(identities, "normalizedProfileSnapshotSha256", "identities.normalizedProfileSnapshotSha256")?
            {
                return Err(contract("normalizedProfileSnapshotSha256 drifted from canonical compiled identities"));
            }
            if text(row, "canonicalPairCompilerAuthoritySha256", "canonicalPairCompilerAuthoritySha256")?
                != text(identities, "pairCompilerAuthoritySha256", "identities.pairCompilerAuthoritySha256")?
            {
                return Err(contract("pair compiler authority drifted from canonical compiled identities"));
            }
            if text(row, "canonicalPairCompileReportSha256", "canonicalPairCompileReportSha256")?
                != text(identities, "validationReportSha256", "identities.validationReportSha256")?
            {
                return Err(contract("pair compile report drifted from canonical compiled identities"));
            }
            if text(row, "nativeValidationAuthoritySha256", "nativeValidationAuthoritySha256")?
                != text(identities, "nativeAuthoritySha256", "identities.nativeAuthoritySha256")?
            {
                return Err(contract("native authority drifted from canonical compiled identities"));
            }
            if text(row, "nativeValidationReportSha256", "nativeValidationReportSha256")?
                != text(identities, "validationReportSha256", "identities.validationReportSha256")?
            {
                return Err(contract("native validation report drifted from canonical compiled identities"));
            }
            if arm == "exact_parent_clone" {
                if !is_null(row.get("historicalParentPairIdentitySha256")) {
                    require_sha(
                        row.get("historicalParentPairIdentitySha256").ok_or_else(|| unexpected("historicalParentPairIdentitySha256"))?,
                        "historicalParentPairIdentitySha256",
                    )?;
                }
                require_bool(
                    ...
                    "compiledV3MatchesHistoricalParent",
                    true,
                    "P receipt must prove compiled v3 identities match the historical parent",
                )?;
            } else {
                if !is_null(row.get("historicalParentPairIdentitySha256")) {
                    return Err(contract("non-parent receipts must not carry historical parent pair identity"));
                }
                if !is_null(row.get("compiledV3MatchesHistoricalParent")) {
                    return Err(contract("non-parent receipts must not claim historical v3 parent match"));
                }
            }
            let parent = parents
            ...
            require_bool(row, "nativeValidationRan", false, "native validation must not be claimed unless it ran")?;
            if !is_null(row.get("canonicalCompiledIdentities")) || !is_null(row.get("frozenPairIdentitySha256")) {
                return Err(contract("incomplete-block receipts cannot claim canonical FrozenPair identities"));
            }
            if !is_null(row.get("frozenPairPayloadSha256")) {
                return Err(contract("incomplete-block receipts cannot claim FrozenPair payloads"));
            }
        }
    }
    ...
    require_text(
        success,
        "schemaVersion",
        "temporal_qd_topology_coadaptation_success_v7",
    ...
    if text(fields, "contractSha256", "contractSha256")? != expected {
        return Err(contract("topology coadaptation v7 identity drift"));
    }
    Ok(payload.clone())
}
