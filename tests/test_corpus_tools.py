...
def _sample_sensitivity_payload(score: float, *, score_lab_version: str = "score_lab_v2_3") -> dict:
    return {
        "data": {
            "aggregate": {
                "score_lab": {
...
def test_score_lab_v2_3_is_canonical_and_legacy_quality_is_diagnostic(tmp_path):
    artifact_dir = tmp_path / "artifact"
    artifact_dir.mkdir()
    _write_json(
        artifact_dir / ct.FULL_BACKTEST_RESULT_FILENAME,
    ...
    assert row["score_basis_36m"] == "score_lab_v2_3:geometric_mean"
