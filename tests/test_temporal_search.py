...
def _plan(profile: dict, start: str, end: str) -> dict:
    ...
    if "managementLibrary" not in execution_config:
        execution_cell_sha256 = canonical_sha256(execution_config["exitPolicy"]["selectedCell"])
    plan = {"schema_version": "fuzzfolio.replay-evidence-plan.v2", "profile_snapshot_sha256": canonical_sha256(profile), "analysis_window_start": start, "analysis_window_end": end, "execution_cell_sha256": execution_cell_sha256, "lake_window_binding": {"window_semantic_sha256": "sha256:" + "b" * 64, "request": {"data_start": "2024-01-01T00:00:00Z", "data_end": "2025-01-01T00:00:00Z", "pairs": ["EURUSD"], "timeframes": ["M5"]}}}
    plan["plan_id"] = canonical_sha256(plan)
    return plan


def _preparation() -> dict:
    profile = _profile(); start = "2024-02-01T00:00:00Z"; end = "2024-03-01T00:00:00Z"
    return {"schemaVersion": "temporal_graph_candidate_window_preparation_v1", "authorityLabel": "stage5d-preflight", "workerContract": {"workerContractSha256": "sha256:" + "c" * 64, "workerContractSchema": "replay-worker-contract-v1"}, "candidates": [{"candidateId": "candidate-a", "sourceProfile": profile, "sourceProfileSha256": canonical_sha256(profile), "instrument": "EURUSD", "timeframe": "M5", "barLimit": 5000, "windowInputs": [{"windowId": "development-a", "evidencePlan": _plan(profile, start, end)}]}], "developmentWindows": [{"windowId": "development-a", "analysisWindowStart": start, "analysisWindowEnd": end}], "prohibitedEvidence": [{"windowId": "reserved-tail", "analysisWindowStart": "2024-06-01T00:00:00Z", "analysisWindowEnd": "2024-07-01T00:00:00Z", "reason": "reserved holdout"}], "bounds": {"maxCandidates": 2, "maxDevelopmentWindows": 2, "maxTasks": 4, "maxAttempts": 2, "deadlineSeconds": 60}}
...
def test_authority_freezes_one_candidate_window_to_one_two_cost_task() -> None:
    ...
    assert task["payload"]["execution_cell"] == _profile()["executionConfig"]["exitPolicy"]["selectedCell"]
    ...
    assert "management.scalar.price_distance.completed_bar" in task["required_worker_capabilities"]
    assert "management.action.dynamic" in task["required_worker_capabilities"]


def test_scalar_management_task_binds_complete_execution_config_without_legacy_cell() -> None:
    ...
    preparation["candidates"][0]["windowInputs"][0]["evidencePlan"] = _plan(profile, start, end)

    payload = build_task_matrix(build_authority(preparation))[0]["payload"]

    assert payload["execution_config_sha256"] == canonical_sha256(profile["executionConfig"])
...
def test_authority_rejects_reserved_overlap_and_profile_plan_mismatch() -> None:
    ...
    identity = dict(plan); identity.pop("plan_id")
    plan["plan_id"] = canonical_sha256(identity)
    with pytest.raises(TemporalSearchContractError, match="profile snapshot mismatch"):
        build_authority(preparation)


def test_plan_checkpoint_is_mutable_but_immutable_manifest_is_not(tmp_path: Path) -> None:
    authority = build_authority(_preparation())
    first = materialize_plan(authority, tmp_path)
    checkpoint = tmp_path / "checkpoint.json"
    state = json.loads(checkpoint.read_text(encoding="utf-8")); state["journal"].append({"taskId": "already-seen"})
    checkpoint.write_text(json.dumps(state), encoding="utf-8")
    assert materialize_plan(authority, tmp_path)["taskMatrixSha256"] == first["taskMatrixSha256"]
...
class _Gateway:
    def __init__(self, task: dict): self.task = task; self.enqueued: list[dict] = []; self.delivered = False; self.acks: list[str] = []
    def enqueue_tasks(self, tasks: list[dict]) -> dict: self.enqueued.extend(tasks); return {"enqueued": len(tasks)}
    def read_results(self, *, limit: int) -> list[dict]:
        if self.delivered or not self.enqueued: return []
        self.delivered = True; job = self.task["payload"]; stream = "sha256:" + "e" * 64
        result = {"schema_version": TEMPORAL_SEARCH_RESULT_SCHEMA, "task_kind": TEMPORAL_SEARCH_TASK_KIND, "job_id": job["job_id"], "authority_id": job["authority_id"], "candidate_id": job["candidate_id"], "evidence_plan_id": job["evidence_plan"]["plan_id"], "lake_window_semantic_sha256": job["lake_window_semantic_sha256"], "shared_observation_stream_id": job["shared_observation_stream_id"], "cost_view_results": {"research_conservative": {"cost_view": "research_conservative", "observation_stream_sha256": stream}, "none": {"cost_view": "none", "observation_stream_sha256": stream}}, "diagnostics": {}, "selection_score": 1.0}
        ...
        for _ in range(16):
            ...
            next_size = len(json.dumps(result, sort_keys=True, separators=(",", ":"), ensure_ascii=True, allow_nan=False).encode("utf-8"))
            if next_size == artifact_size:
                break
            artifact_size = next_size
        return [{"status": "success", "task_id": self.task["task_id"], "lane_id": self.task["lane_id"], "attempt_id": self.task["attempt_id"], "lease_id": "lease-1", "result": {"status": "success", "job_kind": TEMPORAL_SEARCH_TASK_KIND, "result": result}}]
    def ack_results(self, lease_ids: list[str]) -> int: self.acks.extend(lease_ids); return 1


def test_controller_materializes_both_cost_results_from_one_stream(tmp_path: Path) -> None:
    authority = build_authority(_preparation()); task = build_task_matrix(authority)[0]; gateway = _Gateway(task)
    result = run_temporal_search_tasks(gateway, authority, output_root=tmp_path, timeout_seconds=1)
...
def test_result_rejects_different_cost_observation_streams(tmp_path: Path) -> None:
    ...
    def broken(*, limit: int):
        ...
        if rows: rows[0]["result"]["result"]["cost_view_results"]["none"]["observation_stream_sha256"] = "sha256:" + "f" * 64
        return rows
    gateway.read_results = broken  # type: ignore[method-assign]
    with pytest.raises(TemporalSearchContractError, match="identical observation stream"):
